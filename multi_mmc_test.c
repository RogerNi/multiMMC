...
static double binaryMultiMMCPredictionEstimate(const uint8_t *S, long L, const int verbose, const char *label, uint8_t *predictions)
{
 ...
 */
extern "C" double multi_mmc_test(uint8_t *data, long len, int alph_size, const int verbose, const char *label, uint8_t *predictions){
	...
	if(alph_size == 2) return binaryMultiMMCPredictionEstimate(data, len, verbose, label, predictions);
